    ...
    // ----------------------------------------------------
    async function init() {
      // 1. Toujours attacher les écouteurs d'événements UI et d'authentification EN PREMIER
      try {
        setupEventHandlers();
        setupAuthEventHandlers();
      } catch (e) {
        console.error("Erreur lors de l'attachement des écouteurs UI:", e);
      }

      // 2. Initialiser Supabase et la session
      try {
        console.log("Initialisation de l'application Cloud...");
        
        if (!window.supabase) {
          showError("Erreur d'initialisation : La bibliothèque Supabase n'a pas pu être chargée. Vérifiez votre connexion internet.");
          return;
        }
