-- =============================================================================
-- PHARMAOPS - DISPOSABLE ACCOUNT / RLS BEHAVIOR TEST (ROLLBACK ONLY)
-- =============================================================================
-- STATUS: REVIEWED TEST HARNESS - ALWAYS ENDS WITH ROLLBACK.
--
-- This applies the reviewed Steps 2A-2F temporarily inside one transaction,
-- runs the structural and row-count checks, reports PASS, and rolls everything
-- back. It does not permanently change the live schema or application rows.
--
-- IMPORTANT:
--   1. The final ROLLBACK is the core safety control. Never change it to COMMIT.
--   2. Run only after refreshing the live schema and protected baseline.
--   3. Run during a quiet period because DDL may briefly wait on or hold locks.
--   4. This first test verifies migration mechanics. Identity/RLS behavior is
--      tested separately with the disposable account before production.
-- =============================================================================
...
-- TEST MODE: the review-only safety exception is intentionally absent here.
-- This file must retain the final ROLLBACK statement. Never change it to COMMIT.
...
drop policy if exists "Permettre l'insertion ÃƒÆ’Ã‚Â  l'inscription" on public.pharmacies;
...
drop policy if exists "Seul le propriÃƒÆ’Ã‚Â©taire peut lire les achats gÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©riques"
on public.generics_purchases;
drop policy if exists "Seul le propriÃƒÆ’Ã‚Â©taire peut ÃƒÆ’Ã‚Â©crire les achats gÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©riques"
on public.generics_purchases;
...
-- =============================================================================
-- DISPOSABLE ACCOUNT AND CROSS-PHARMACY BEHAVIOR TESTS
-- =============================================================================
-- Existing disposable identity:
--   user     7568770a-17db-4ac0-83e5-30479002777f
--   pharmacy 673cbff2-00f0-46f5-9b3a-47d292fb706a
--
-- Protected production identity:
--   user     c37f945e-2c4d-4449-9268-d7e8848e7237
--   pharmacy 1602e82f-5ee5-4255-ace8-a7644ad3db40
-- =============================================================================

-- Test the disposable pharmacy as its authenticated owner.
set local role authenticated;
set local request.jwt.claim.sub = '7568770a-17db-4ac0-83e5-30479002777f';
...
begin
  if exists (
    ...
    where id <> '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid
  ) then
    raise exception 'RLS TEST FAILED: disposable owner can see another pharmacy';
  end if;

  if exists (
    select 1 from public.profiles
    where pharmacy_id is distinct from
      '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid
  ) then
    raise exception 'RLS TEST FAILED: disposable owner can see another profile';
  end if;

  if exists (
    select 1 from public.configurations
    where pharmacy_id <>
      '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid
  ) or exists (
    select 1 from public.flagged_products
    where pharmacy_id <>
      '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid
  ) or exists (
    select 1 from public.generics_purchases
    where pharmacy_id <>
      '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid
  ) then
    raise exception 'RLS TEST FAILED: disposable owner can see private rows from another pharmacy';
  end if;

  if exists (
    select 1 from public.catalog
    where pharmacy_id not in (
      '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid,
      '1602e82f-5ee5-4255-ace8-a7644ad3db40'::uuid
    )
  ) then
    raise exception 'RLS TEST FAILED: disposable owner can see a non-own/non-master catalog';
  end if;

  update public.configurations
  set updated_at = updated_at
  where pharmacy_id = '1602e82f-5ee5-4255-ace8-a7644ad3db40'::uuid;
  get diagnostics v_rows = row_count;
  if v_rows <> 0 then
    raise exception 'RLS TEST FAILED: disposable owner updated production configuration';
  end if;

  begin
    update public.profiles
    set role = 'owner'
    where id = '7568770a-17db-4ac0-83e5-30479002777f'::uuid;
  exception
    when insufficient_privilege then
      v_rejected := true;
  end;
  if not v_rejected then
    raise exception 'RLS TEST FAILED: browser could directly update a security role';
  end if;
  ...
  get diagnostics v_rows = row_count;
  if v_rows <> 1 then
    raise exception 'RLS TEST FAILED: disposable owner cannot update own invitation code';
  end if;

  v_rejected := false;
  begin
    perform public.set_pharmacy_member_role(
      'c37f945e-2c4d-4449-9268-d7e8848e7237'::uuid,
      'tech'
    );
  exception
    when others then
  ...
  if not v_rejected then
    raise exception 'RLS TEST FAILED: disposable owner managed production owner';
  end if;
end;
$$;
...
-- Test the protected owner sees no disposable pharmacy-private rows.
set local role authenticated;
set local request.jwt.claim.sub = 'c37f945e-2c4d-4449-9268-d7e8848e7237';
...
begin
  if exists (
    ...
    raise exception 'RLS TEST FAILED: production owner can see another pharmacy';
  end if;
  ...
  if exists (
    ...
    raise exception 'RLS TEST FAILED: production owner can see another profile';
  end if;
  ...
  if exists (
    ...
    raise exception 'RLS TEST FAILED: production owner can see disposable private rows';
  end if;

  update public.configurations
  set updated_at = updated_at
  where pharmacy_id = '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid;
  get diagnostics v_rows = row_count;
  if v_rows <> 0 then
    raise exception 'RLS TEST FAILED: production owner updated disposable configuration';
  end if;
end;
$$;
...
-- Test controlled join and create onboarding with the disposable profile.
select set_config(
  'pharmaops.test_join_code',
  (
    select code::text
    from public.pharmacies
    where id = '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid
...
do $$
begin
  perform public.join_pharmacy_for_current_user(
    current_setting('pharmaops.test_join_code'),
    'TST'
  );
  ...
  if not exists (
    ...
    from public.profiles
    where id = '7568770a-17db-4ac0-83e5-30479002777f'::uuid
      and pharmacy_id = '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid
      and role = 'tech'
  ) then
    raise exception 'ONBOARDING TEST FAILED: controlled join did not assign technician';
  end if;
end;
$$;
...
declare
  v_new_pharmacy_id uuid;
begin
  v_new_pharmacy_id := public.create_pharmacy_for_current_user(
    'ROLLBACK TEST PHARMACY',
    'TST'
  );

  if v_new_pharmacy_id is null
     or v_new_pharmacy_id in (
       '673cbff2-00f0-46f5-9b3a-47d292fb706a'::uuid,
       '1602e82f-5ee5-4255-ace8-a7644ad3db40'::uuid
     )
     or not exists (
       ...
       from public.profiles
       where id = '7568770a-17db-4ac0-83e5-30479002777f'::uuid
         and pharmacy_id = v_new_pharmacy_id
         and role = 'owner'
     )
  then
    raise exception 'ONBOARDING TEST FAILED: controlled create did not assign owner';
  end if;
end;
$$;
...
-- A visible success row appears only if every migration and postcheck statement
-- succeeded. ROLLBACK then restores the original production schema and data.
select
  'PASS - migration, isolation, onboarding, and rollback checks succeeded' as rollback_test_result;

rollback;

-- =============================================================================
-- REQUIRED BEHAVIORAL TESTS BEFORE PRODUCTION
-- =============================================================================
-- 1. Run the unchanged draft and confirm DRAFT ONLY aborts with no schema/data
--    changes.
-- 2. Test signup, create-pharmacy, and join-pharmacy with disposable accounts.
-- 3. Test owner and technician permissions within the disposable pharmacy.
-- 4. Test that neither account can read or write another pharmacy's private rows.
-- 5. Test monthly and annual generic dashboards: save, reload, and render.
-- 6. Verify anonymous access and direct security-field updates are denied.
-- 7. Recheck PJC 28 owner identity and protected row counts before and after.
-- =============================================================================

-- =============================================================================
-- EXPECTED RESULT
-- =============================================================================
-- The SQL Editor reports PASS and ROLLBACK. Afterward, the live policies,
-- functions, grants, profiles, pharmacies, and application row counts must be
-- identical to their pre-test state.
-- =============================================================================
