-- =============================================================================
-- PHARMAOPS - CONSOLIDATED SECURITY MIGRATION 2A-2F (REVIEW DRAFT ONLY)
-- =============================================================================
-- STATUS: DO NOT RUN YET.
--
-- This is the single ordered migration assembled from the reviewed Steps 2A-2F.
-- It changes authorization, RLS policies, grants, and controlled RPC functions.
-- It does not delete or rewrite inventory, expiry, configuration, or dashboard
-- rows. All work is inside one transaction, so any error rolls everything back.
--
-- IMPORTANT:
--   1. Keep the DRAFT ONLY safety stop until disposable-account testing.
--   2. Refresh the live schema and protected baseline immediately before use.
--   3. Deploy the matching App/index.html in coordination with this migration.
-- =============================================================================
...
-- Deliberate safety stop. Running this review draft raises an exception and
-- PostgreSQL rolls back the transaction without applying any following change.
do $$
begin
  raise exception
    'DRAFT ONLY: disposable-account test and final approval are required';
end;
$$;
